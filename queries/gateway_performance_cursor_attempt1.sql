...
-- txn and invoice success rates by timeframe (day/week/month/quarter). Bin breakdown limited to top 100 by volume.
...
, invoice_success as
    (
        select
            ...
            , invoice_grouper
            , max(case
                when trans_type_desc = 'purchase'
                    and trans_status_desc in ('success', 'void')
                    then 1
                when trans_type_desc = 'verify'
                    and trans_status_desc in ('success', 'void')
                    then 1
                else 0
...
-- Transaction-level aggregation: by gateway, timeframe grains, cc_first_6_nbr, transaction_category.
, txn_metrics as
    (
        select
            trans_gateway_type_desc
            , date(trans_dt) as trans_dt_day
            , date_trunc(trans_dt, week) as trans_dt_week
            , date_trunc(trans_dt, month) as trans_dt_month
            , date_trunc(trans_dt, quarter) as trans_dt_quarter
            , cc_first_6_nbr
        ...
        group by
            trans_gateway_type_desc
            , date(trans_dt)
            , date_trunc(trans_dt, week)
            , date_trunc(trans_dt, month)
            , date_trunc(trans_dt, quarter)
            , cc_first_6_nbr
            , transaction_category
    )
...
-- Day-level combined metrics: full outer join txn + inv at day grain; top 100 bins (overall) only.
-- Week/month/quarter produced in final SELECT by aggregating from this.
, metrics_day as
    (
        select
        ...
        from 
            (
                ...
                from txn_metrics
                where 1=1
                    and cc_first_6_nbr in (select cc_first_6_nbr from top_100_bins)
            ) t
        full outer join
            (
                select
...
, full_results as
    (
        select
            ...
            , dt as timeframe_value
            , trans_gateway_type_desc
            , cc_first_6_nbr
            , category
            , total_transactions
            , successful_transactions
            , safe_divide(successful_transactions, nullif(total_transactions, 0)) as txn_success_rate
            , total_invoices
            , successful_invoices
            , safe_divide(successful_invoices, nullif(total_invoices, 0)) as inv_success_rate
        from metrics_day
        where 1=1
        union all
        select
            ...
            , safe_divide(sum(successful_transactions), nullif(sum(total_transactions), 0))
            , sum(total_invoices)
            , sum(successful_invoices)
            , safe_divide(sum(successful_invoices), nullif(sum(total_invoices), 0))
        from metrics_day
        where 1=1
        group by
            date_trunc(dt, week)
