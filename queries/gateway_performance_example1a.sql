...
/*
Need to update this, probably a union of invoice level & txn level
Currently, having coalesce(invoice_billed_dt_ut, trans_dt_ut) is not correct for txn level 
    as txn's get assigned to different months based on the related invoice

add in a metric_level field that will be either 'txn' or 'invoice'
update aggregate field names to be generic & the metric level will indicate which level is being aggregated
