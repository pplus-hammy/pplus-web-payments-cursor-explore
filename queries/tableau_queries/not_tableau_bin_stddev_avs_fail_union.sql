...
with txn as
    (
        ...
        where 1=1
            ...
            and txn.trans_dt >= date_add(current_date(), interval -6 month)
...
, run_dates as
    (
        select run_dt
        from unnest(generate_date_array(date_sub(current_date(), interval 3 month), current_date())) as run_dt
    )
...
, daily_volume_all as
    (
        ...
        from txn
        where 1=1
            and trans_dt >= date_sub(current_date(), interval 3 month)
            and trans_dt <= current_date()
            -- and trans_dt not in (date('2026-01-24'),date('2026-01-25'),date('2026-01-31'), date('2026-02-01'),date('2026-02-28')) --exclude big event days, which skew the deviations
...
, dow_baseline as
    (
        ...
        where 1=1
            -- and bd.baseline_dt not in (date('2026-01-24'),date('2026-01-25'),date('2026-01-31'), date('2026-02-01'),date('2026-02-28'))
...
, daily_volume as
    (
        ...
        from daily_volume_all
        where 1=1
            and trans_dt between date_add(current_date(), interval -3 month) and current_date()
    )
...
, chg_chk as
    (
        select
            ...
            , case
                when (dv.daily_ct - bl.baseline_avg_ct) / nullif(bl.baseline_stddev_ct, 0) >= 3 then 'large_increase'
                when (dv.daily_ct - bl.baseline_avg_ct) / nullif(bl.baseline_stddev_ct, 0) <= -3 then 'large_decrease'
                else null
            end as chg_flag
            ...
            , case
                when (dv.daily_success_ct - bl.baseline_success_avg_ct) / nullif(bl.baseline_success_stddev_ct, 0) >= 3 then 'large_increase'
                when (dv.daily_success_ct - bl.baseline_success_avg_ct) / nullif(bl.baseline_success_stddev_ct, 0) <= -3 then 'large_decrease'
                else null
            ...
            , case
                when (dv.daily_decline_ct - bl.baseline_decline_avg_ct) / nullif(bl.baseline_decline_stddev_ct, 0) >= 3 then 'large_increase'
                when (dv.daily_decline_ct - bl.baseline_decline_avg_ct) / nullif(bl.baseline_decline_stddev_ct, 0) <= -3 then 'large_decrease'
                else null
