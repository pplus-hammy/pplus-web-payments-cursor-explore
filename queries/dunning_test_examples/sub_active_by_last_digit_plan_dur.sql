-- Active subscriptions by account_cd last digit (0–9) and plan_dur (monthly / annual).
-- Uses computed status_desc and plan_dur from sub.sql; 10 rows via digit spine.
...
, agg as
    (
        select
            ...
            , count(distinct case when plan_dur = 'monthly' then subscription_guid end) as monthly_active_sub_cnt
            , count(distinct case when plan_dur = 'annual' then subscription_guid end) as annual_active_sub_cnt
        from sub
        where 1=1
            and status_desc = 'active'
            and account_cd is not null
            and regexp_contains(right(trim(cast(account_cd as string)), 1), r'^[0-9]$')
        group by
            account_cd_last_digit
    )
select
    digits.account_cd_last_digit
    , coalesce(agg.monthly_active_sub_cnt, 0) as monthly_active_sub_cnt
    , round(100.0 * coalesce(agg.monthly_active_sub_cnt, 0) / nullif(sum(coalesce(agg.monthly_active_sub_cnt, 0)) over (), 0), 4) as monthly_pct_of_monthly_total
    , coalesce(agg.annual_active_sub_cnt, 0) as annual_active_sub_cnt
    , round(100.0 * coalesce(agg.annual_active_sub_cnt, 0) / nullif(sum(coalesce(agg.annual_active_sub_cnt, 0)) over (), 0), 4) as annual_pct_of_annual_total
from digits
left join agg
    on agg.account_cd_last_digit = digits.account_cd_last_digit
order by cast(digits.account_cd_last_digit as int64)
