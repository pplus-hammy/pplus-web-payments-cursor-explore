...
, dates_with_volume as
    (
        select
            ...
            , coalesce(dv.daily_ct, 0) as daily_ct
            , coalesce(dv.daily_success_ct, 0) as daily_success_ct
            , coalesce(dv.daily_decline_ct, 0) as daily_decline_ct

            , coalesce(dv.daily_success_avs_fail_ct, 0) as daily_success_avs_fail_ct
        from bins_and_dates bd
        left join daily_bin_volume dv
            on bd.src_system_id = dv.src_system_id
            and bd.gateway_country = dv.gateway_country
            and bd.cc_first_6_nbr = dv.cc_first_6_nbr
            and bd.dt = dv.trans_dt
        where 1=1
    )
...
, baseline_with_volume as
    (
        select
            ...
            , dwv.cc_first_6_nbr
            , bd.run_dt 
            , bd.baseline_dt -- list of dates with the same day of week over the past x months (ex: run_dt = Jan31 would have baseline_dts = Jan24, Jan17, Jan10, Jan3, etc)
            , dwv.daily_ct
